	...
	Action: func(cctx *cli.Context) error {
		...
		if avail.Channel == nil {
			if avail.PendingWaitSentinel != nil {
				fmt.Fprintf(cctx.App.Writer, "Creating channel\n")
			...
			fmt.Fprintf(cctx.App.Writer, "Channel does not exist\n")
			fmt.Fprintf(cctx.App.Writer, "  From: %s\n", from)
			fmt.Fprintf(cctx.App.Writer, "  To:   %s\n", to)
			return nil
		}
		...
		if avail.PendingWaitSentinel != nil {
			fmt.Fprintf(cctx.App.Writer, "Adding Funds to channel\n")
		} else {
			fmt.Fprintf(cctx.App.Writer, "Channel exists\n")
		}
		...
		fmt.Fprintf(cctx.App.Writer, formatNameValues(nameValues))
		return nil
	},
}
